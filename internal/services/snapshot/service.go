...
// Гибридный подход: GetStatistics для точных данных + GetAllUnitsWithStatus для деактивированных
func (s *Service) createSnapshotsForConnection(wialonClient *wialon.Client, accounts []models.Account, snapshotDate time.Time) ([]models.Snapshot, error) {
	...
	// 1. Получаем статистику через GetStatistics API (точные данные)
	...
	if err != nil {
		log.Printf("createSnapshotsForConnection: ошибка GetStatistics: %v, используем fallback", err)
		return s.createSnapshotsViaUnits(wialonClient, accounts, snapshotDate)
	}

	// 2. Получаем все объекты с информацией о деактивации
	...
	if err != nil {
		...
		// Продолжаем без данных о деактивации
		unitsResp = nil
	}
	...
	for _, account := range accounts {
		...
		var totalUnits, unitsCreated, unitsDeleted int
		if len(accountStats) > 0 {
			totalUnits = accountStats[0].UnitTotal
			unitsCreated = accountStats[0].UnitCreated
			unitsDeleted = accountStats[0].UnitDeleted
		}

		// Получаем количество деактивированных для этого аккаунта
